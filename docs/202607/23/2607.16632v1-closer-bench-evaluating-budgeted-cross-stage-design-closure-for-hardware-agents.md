...
score: 7.0
evidence: 硬件智能体长时域任务基准
tldr: 硬件代理的跨阶段设计收敛评估缺乏统一基准。CLOSER-Bench提出预算控制下的规范到RTL、RTL到GDS及规范到GDS配对任务，记录仿真、综合、时序分析等工具调用次数，衡量最终质量、任意时刻进度、工具成本和跨阶段恢复能力。基于开源工具链的十任务试点发现，代理在局部AXI修复任务上表现良好，但面对验证收敛任务时表现分化，揭示了完成与收敛之间的显著差距。该工作倡导将硬件收敛视为预算顺序决策问题，而非独立代码生成任务。
source: arxiv
...
motivation: 现有基准测试孤立评估RTL生成、验证或物理实现，缺乏跨阶段统一协议，难以诊断代理在抽象边界上的成败原因。
method: 提出CLOSER-Bench，为同一设计和隐藏目标配对spec-to-RTL、RTL-to-GDS和spec-to-GDS任务，记录每次工具调用并计算质量、进度、成本和恢复能力。
result: 十任务试点中，三个代理成功完成AXI修复，但验证收敛任务将前沿代理与其他基线分离，暴露了完成与收敛之间的差距。
conclusion: 硬件收敛应被建模为预算顺序决策问题，而非独立代码生成任务，以指导未来代理设计。
---

## 摘要
硬件工程使得代码智能体面临一种难以用pass-at-k捕捉的长期工作形式：进展是连续的，工具反馈是延迟且异构的，后端失败可能需要修改RTL而非调整另一个物理设计参数。现有基准测试衡量RTL生成、仓库修复、验证、PPA演进或物理实现，但它们不同的设计和预测器使得难以确定智能体在抽象边界上的成功或失败点。我们引入CLOSER-Bench，一个用于预算约束跨阶段设计收敛的受控评估协议。对于一个设计和隐藏目标，它配对spec-to-RTL、RTL-to-GDS和spec-to-GDS任务，记录每次仿真器、综合、STA和布局布线调用，并衡量最终质量、随时进展、工具成本和跨阶段恢复。该基准测试基于开源Verilator、Yosys、OpenROAD、KLayout、Sky130和Harbor智能体框架构建。一个涵盖RTL修复、基于突变的验证、覆盖率、PPA优化、设计空间探索、跨模型调试和安全性的十任务试点建立了可执行框架，并揭示了尖锐的完成-收敛差距：三个智能体解决了局部AXI修复任务，而匹配的验证收敛任务将一个前沿智能体与两个原本成功的基线区分开。我们进一步验证了完整的RTL-to-GDS流程，并为阶段配对评估构建了一个基于宏的AXI/DMA流加速器。这些结果激励将硬件收敛视为一个预算约束的序列决策问题，而非一组独立的代码生成任务。
